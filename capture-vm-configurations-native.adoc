...
[.lead]
You can use the Migration advisor VM collector script, RVTools, or NetApp Data Infrastructure Insights to capture your current VM configurations. We recommend that you use the Migration advisor VM collector script because it gathers both VM configuration and performance data for the most accurate plan.
...
. In the menu, select *Admin*.
. In the resulting menu, select *API Access*.
. Ensure that *API Access Tokens* is enabled and select *Add API Token*.
...
. Save this token in preparation to use with the migration advisor.
