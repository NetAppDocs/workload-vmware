...
summary: NetApp workload factory for VMware provides tools to move your data from on-premises systems to either VMware Cloud on AWS (VMC) or Amazon EC2. 
---
...
[.lead]
NetApp workload factory for VMware provides tools to move your data from on-premises systems to either VMware Cloud on AWS (VMC) or Amazon EC2.

== What is NetApp workload factory for VMware?

NetApp workload factory for VMware provides a migration advisor that enables you to analyze your current virtual machine configurations in on-premises vSphere environments. The migration advisor then generates a plan to deploy recommended VM layouts to Amazon EC2 or VMware Cloud on AWS vSphere clusters and use customized Amazon FSx for NetApp ONTAP file systems as external datastores. 
