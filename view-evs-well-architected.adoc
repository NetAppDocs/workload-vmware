...
= View the Well-architected dashboard for EVS environments
:icons: font
:imagesdir: ./media/

[.lead]
The Well-architected dashboard provides automated analysis of your Amazon Elastic VMware Service (EVS) environments to ensure alignment with AWS and NetApp best practices. The dashboard identifies configuration issues and provides detailed guidance on issue resolution to help you maintain optimal operations, security, and cost efficiency.

== Overview

The Well-architected dashboard performs automatic daily scans of all discovered EVS environments using AWS APIs. These scans analyze your EVS configuration and identify potential issues that could impact availability, resiliency, security, or cost optimization. Findings are organized by configuration area, with each finding including status, severity levels, impacted resource details, and step-by-step remediation procedures.
...
* *View-only insights*: The MVP release provides detailed findings and recommendations without automated issue resolution options.
...
+
The Well-architected dashboard is displayed with the following elements:
...
The Well-architected dashboard displays findings in topic cards:

Each topic card displays:

* *Configuation name*: The configuration area being assessed
...
This subcategory includes details related to the configuration of your EVS environment and its resources.
...
. In the Well-architected dashboard, locate *Cluster node management*.
...
. In the Well-architected dashboard, locate *EVS environment resiliency*.
